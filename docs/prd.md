...
- **Build the list** — Add a med in natural language; yes/no or corrections when the assistant needs confirmation; sometimes answer how many days ahead to schedule reminders after a save. When someone already has other meds on file, the assistant reply can include an extra **cross-check** for possible interactions (reference-grounded, disclaimer-heavy — not a substitute for a clinician or pharmacist).
