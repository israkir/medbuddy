...
- Webhook endpoint accepts verified LINE events when `LINE_CHANNEL_SECRET` is set; mock mode may skip signature verification (see backend README). **`POST /v1/line/webhook`** returns after parse/idempotency and schedules each event on FastAPI **`BackgroundTasks`** (slow work runs off the HTTP thread). When **`REDIS_URL`** is set, **`idempotency.py`** deduplicates retries by **`webhookEventId`** (short TTL).
...
- **Inbound text and voice** start LINE’s **chat loading indicator** (`send_chat_loading_indicator`) before the assistant path (text: before LLM; voice: before download/STT).
...
- **`GET /v1/app/me`** — `app_user_id` and profile: `preferred_name`, `age_years`, `gender`, `emergency_contacts` (list), `health_conditions` (list of structured rows: `id`, `category`, `name`, `severity`, `notes`, `is_active`), **`locale`** (`en` \| `zh-TW`, default `zh-TW`), **`timezone`** (IANA, default `Asia/Taipei`), `onboarding_completed_at`. Until onboarding is completed, optional **`X-MedBuddy-Locale`** or **`Accept-Language`** may sync **`locale`** to the client (see [`tdd-extended.md`](tdd-extended.md) §7 **`GET /v1/app/me`**).
- **`POST /v1/app/onboarding`** — Persists onboarding via `UserDataPort.save_onboarding_profile`; required `preferred_name`; optional demographics, emergency contact, `health_conditions: list[HealthConditionOnboardingItem]`, optional IANA **`timezone`**, optional **`locale`** (standalone app typically sends device language choice).
...
| Bulk / journal / notify / health | `remove_all_medications`, `disable_reminders`, `export_health_journal`, `simulate_notify_emergency_contact`, **`manage_health_conditions`**, **`lookup_health_history`**, **`update_profile`** | Wired in `orchestrator.py` → tools / `extract_profile_patch` / health-condition stores |
...
| **Reminder horizon** | After save when `needs_horizon_confirmation` on the draft (**finite meds only** — `is_indefinite` extractions skip this) | `try_resolve_pending_reminder_horizon` | User sends a **day count** (or week phrasing); metadata patched and reminders re-synced. |
...
| **Capabilities** | Extraction via `LLMPort.extract_medication_draft` → `MedicationDraft`. Missing drug name → `MedicationExtractionError` → i18n `medication.add_incomplete`. **Duplicate guard:** case-insensitive match against an existing med name → i18n `medication.already_on_file` (no new row). The extractor also returns **`is_indefinite`** (chronic phrasing such as *"long-term"*, *"every day from now on"*, *"長期"*, *"終身"*, *"慢性病用藥"*); when set, `medication_draft_from_extraction` forces `needs_horizon_confirmation=false` and `materialize_daily_reminders=true`, so chronic / lifelong meds skip the "how many days?" follow-up entirely and the post-save reply uses the dedicated `llm.added_indefinite` locale string. When `medication_draft_needs_add_confirmation` (placeholder dose/schedule, missing user-evidence for dose+schedule in the utterance, horizon confirmation, etc.): store **`MedicationAddConfirmationPending`**, reply with prose **`medication.add_confirm_prompt`** (no DB row yet). **Yes** path persists via `persist_medication_add_from_draft` (sync reminders, optional **`ReminderHorizonPending`** if `needs_horizon_confirmation` — **never** set when `is_indefinite=true`). **Saved** path: reload list, `DrugDataPort` snippets for the **new** drug, then **`build_post_add_patient_reply`** (`application/post_add_medication_reply.py`) with `patient_context_for_llm` (`include_health_notes=True`). **First medication on an empty list:** **`compose_medication_added_reply`**. **When the patient already had ≥1 other medication:** **`compose_medication_added_primary`**, then **`post_add_interaction_crosscheck`** (continuation-focused prompt — not chat **`check_interactions_structured`**) with synthetic `medication.post_add_interaction_user_query`, appended after **`medication.post_add_interaction_bridge`**. **`check_drug_condition_interactions`** may append moderate/high drug–condition lines when active **`patient_health_conditions`** exist. Just-in-time education lines append only when compose falls back to i18n `medication.added`. Failures in the cross-check are logged and do not block the save. |
| **Implementation** | `agents/tools/medication_crud.py` → `persist_medication_add_from_draft`. Successful add/update/removal triggers `sync_and_enqueue_reminders` when wired (§8). Indefinite meds also opt the row into the daily chronic resync cron and the delivery-time top-up safety net (§8). |
...
| Patient context for LLM | `patient_context_for_llm` (calls `sync_upcoming_dose_events` + `list_upcoming_dose_events`, then `build_patient_context_for_llm` with appended block) — same de-identified profile signals and medication lines as before, plus **clock-ordered pending `dose_events`** for ~7 days from local midnight; not raw free-text from `patient_health_conditions` entries, raw emergency contact values, or exact `age_years`. The `recent_health_notes` buffer (assembled from recent `patient_health_conditions` rows, not a removed stored column) is included only when `include_health_notes=True` — interaction-check, side-effects, health-summary, and post-add compose call sites. |
...
| Medications | `medications` | Per-patient list for assistant and reminders. The dedicated **`is_indefinite boolean`** column marks chronic / lifelong rows (partial index `medications_is_indefinite_idx`) and is read by both the daily chronic resync cron and the delivery-time top-up safety net (§8). |
...
| Extraction | On add, the LLM can return structured **reminder preferences** (e.g. first reminder in N minutes, daily horizon days, whether to fan daily rows, optional local time) plus **`is_indefinite`** for chronic / lifelong meds. Stored under **`medications.raw_metadata.reminder`** plus the dedicated **`medications.is_indefinite`** column, and consumed when building `dose_events` (e.g. “in 5 minutes” → a single upcoming instant without fanning the full horizon). Env defaults `MEDBUDDY_REMINDER_*` apply when fields are unset. |
...
| Chronic / indefinite-duration meds | When `medications.is_indefinite = true`, the rolling window is **refilled** by two mechanisms instead of expiring: (1) `resync_chronic_meds_cron` runs daily (default **03:15 UTC**, configurable via `MEDBUDDY_CHRONIC_RESYNC_CRON_HOUR_UTC` / `MEDBUDDY_CHRONIC_RESYNC_CRON_MINUTE_UTC`) and calls `resync_all_indefinite_patients` → `sync_and_enqueue_reminders` for every patient with at least one chronic med (selected via `UserDataPort.list_patients_with_indefinite_medications`, backed by the `medications_is_indefinite_idx` partial index); (2) `deliver_dose_reminder` calls `_maybe_topup_chronic_med` after a successful push and, when fewer than **`MEDBUDDY_CHRONIC_DELIVERY_TOPUP_THRESHOLD`** (default **3**) future events remain for that med, fires `sync_and_enqueue_reminders` immediately. Saves of chronic meds skip the "how many days?" question and use the `llm.added_indefinite` locale string instead of the day-count copy. |
| Delivery | With Redis, `enqueue_reminder_jobs` schedules arq `send_reminder_for_dose` with `_defer_until = scheduled_at`. Worker runs `deliver_dose_reminder` → LINE `push_message`, then `reminder_sent_at`; for indefinite meds, the same path also runs the chronic top-up check. |
...
| Copy | Primary: **`reminder.line_push`** (`zh-TW`, `en`); welcome and pushes respect **`patients.locale`** (LINE follow may seed locale from LINE profile **`language`** first). Chronic save reply uses **`llm.added_indefinite`** instead of the day-count message. |
...
| Reconcile | `POST /internal/reminders/reconcile` with `X-Cron-Secret`. The reconcile path covers *missed pushes for already-materialized rows*; the chronic resync cron is the complementary safety net for *missing future doses* on indefinite meds. |
