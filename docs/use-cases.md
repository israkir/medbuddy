...
**Example:** 「我的藥清單」 / `list my medications`
...
**Example:** User opens the in-app chat screen and sends “What is metformin for?” (or taps a suggested prompt).
...
4. **Reference data:** **`DrugDataPort`** fetches snippets for the **new drug name** (OpenFDA / TFDA placeholder / mocks), same family of grounding used for explain—**not** the per-user **`drug_personalization_cache`** (that cache remains for **`explain_medication`** / **`interaction_check`** only).
...
3. Else **reference data:** **`DrugDataPort`** (HTTP OpenFDA label search and TFDA placeholder, or mocks). With Supabase, **`CachingDrugData`** reads/writes **`drug_reference_cache`** (TTL configurable).
...
**Examples:** Dose timing check, 「血壓 130」-style note, 「給我摘要」, small talk.
...
| Personalization | `drug_personalization_cache` | Per-user LLM answer for explain/interaction fingerprints |
...
- Full TFDA API integration (TFDA path is still placeholder / mock-friendly).
