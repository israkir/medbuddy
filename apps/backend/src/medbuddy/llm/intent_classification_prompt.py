...
- **emergency** — User is reporting a **life-threatening medical situation** requiring immediate action: chest pain or tightness, difficulty breathing or can't breathe, suspected stroke (sudden face drooping, arm weakness, slurred speech, sudden severe headache), severe allergic reaction (throat swelling, can't breathe after eating/taking a pill), loss of consciousness or fainting, severe uncontrolled bleeding, poisoning or overdose, or any situation where the user or a bystander says "help" or "emergency" in a medical context. **Choose this before any other intent when life-threatening signals are present.** Do not use it for mild side effects, general discomfort, or chronic symptom discussions.
...
- **list_medications** — User asks what medications they have on file / their list / "what am I taking" (static list — not a clock-time schedule).
- **upcoming_doses** — User wants a **time-ordered schedule** from reminders: what to take **soon**, **later**, **next**, the **rest of today**, **today's doses**, **this week**, or any wording focused on **when** (not merely which drugs exist). Examples: "what should I take later", "what's my schedule today", "what's next", "接下來要吃什麼藥", "今天還要吃哪些". Server reads materialized `dose_events` (authoritative times), not guessed frequency text alone.
...
- **explain_medication** — User wants educational information about a drug: what it's for, how it works, how/when to take it, side effect profile, timing rationale — but NOT adding it to their list, NOT reporting a side effect they are currently experiencing. E.g. "what is metformin for", "why do I take this pill", "what are the side effects of aspirin", "can I take it with food".
- **report_side_effects** — User is **currently experiencing** a symptom or side effect that they believe is caused by a medication they are taking. Different from explain_medication (asking about hypothetical effects) and from confirm_dose (logging adherence). Use this when the user says things like: "since I started taking X I feel Y", "my medication is making me dizzy", "I think aspirin is causing my stomach pain", "I've been taking metformin for a week and I feel nauseous". Do NOT use for life-threatening symptoms — use **emergency** instead.
...
- **update_profile** — Durable profile fields: how to address the user, age, gender, emergency contacts, allergies, long-term health notes, and user preferences like reply language (`en`/`zh-TW`) or timezone. One-off symptoms **not** meant as a dose log belong here or **general_question**, not confirm_dose, unless they explicitly tie it to a dose log (then use confirm_dose + **dose_adherence_note**). Include lines that give one or multiple emergency/family contacts with channels (e.g. phone/email/LINE/WhatsApp) — **not** add_medication.
...
- **emergency takes absolute priority** — If any message contains life-threatening signals (chest pain, can't breathe, stroke signs, severe allergic reaction, loss of consciousness, suspected poisoning), classify as **emergency** immediately, regardless of other content.
- Prefer **report_side_effects** over **general_question** when the user is currently experiencing a symptom they attribute to a medication they take. Prefer **explain_medication** over **report_side_effects** when they are asking hypothetically about a drug's side effects without currently experiencing them.
...
- Prefer **upcoming_doses** over **list_medications** when the question is about **timing / order / what's due** (soon, later, today, next dose). Prefer **list_medications** when they only want the **inventory** of medicines without a schedule.
- Prefer **update_profile** over **general_question** when the user states **their own** name, age, or gender to fill their record (self-introduction, onboarding, “call me…”, “I am … years old”) — not when they are only naming an emergency contact or family member.
- Requests to change assistant reply language (e.g. "speak English", "can you talk to me in English", "請用中文") should be **update_profile** (locale preference), not **general_question** or **off_topic**.
...
- When the user's message contains **both** a dose-taken signal (吃了, 吃完, took it, swallowed, 服用了) **and** a symptom or side effect (拉肚子, 頭痛, 噁心, dizzy, stomach ache…), classify as **report_side_effects** (to give contextual advice) but **always** set **record_pending_dose_as_taken=True** and **dose_adherence_note** to the symptom text. This ensures the symptom is stored on the dose record for the doctor even while the user also receives advice.
